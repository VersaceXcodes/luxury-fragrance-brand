...
const UV_ProductDetail: React.FC = () => {
  ...
  const { data: wishlists = [] } = useQuery({
    ...
    queryFn: async (): Promise<Wishlist[]> => {
      const response = await axios.get(
        `${import.meta.env.VITE_API_BASE_URL || 'https://123luxury-fragrance-brand.launchpulse.ai'}/api/wishlists`,
        {
          headers: {
  ...
  const addToWishlistMutation = useMutation({
    mutationFn: async (wishlistId: string) => {
      ...
      const response = await axios.post(
        `${import.meta.env.VITE_API_BASE_URL || 'https://123luxury-fragrance-brand.launchpulse.ai'}/api/wishlists/${wishlistId}/items`,
        {
          product_id: product.product_id,
  ...
  const handleAddToWishlist = () => {
    ...
    // If user has wishlists, show selector
