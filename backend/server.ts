...
const sessionConnections = new Map<string, Set<ExtendedWebSocket>>();
...
app.get('/api/products', async (req, res) => {
  try {
    ...
    if (query) {
      ...
      const fragranceFamilyKeywords: Record<string, string[]> = {
  ...
  try {
    ...
    if (validatedData.is_default) {
      await client.query(
        ...
        [userId, validatedData.address_type]
      );
    }
    ...
    const result = await client.query(
      `INSERT INTO addresses (address_id, user_id, address_type, first_name, last_name, company, 
       ...
       RETURNING *`,
      [addressId, userId, validatedData.address_type, validatedData.first_name, 
...
*/
app.post('/api/chat/start', optionalAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    // Create chat session
    await client.query(
      `INSERT INTO chat_sessions (session_id, user_id, status, topic, created_at, updated_at) 
       VALUES ($1, $2, $3, $4, $5, $6)`,
      [sessionId, req.user?.user_id || null, 'waiting', topic || 'general', now, now]
    );
    ...
    if (message) {
      ...
      await client.query(
        `INSERT INTO chat_messages (message_id, session_id, sender_id, sender_type, message, created_at) 
         VALUES ($1, $2, $3, $4, $5, $6)`,
        [messageId, sessionId, req.user?.user_id || 'guest', 'user', message, now]
      );
    }
...
*/
app.get('/api/chat/:session_id', optionalAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
...
*/
app.post('/api/chat/:session_id/end', optionalAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { session_id } = req.params;
    ...
    const result = await client.query(
      'UPDATE chat_sessions SET status = $1, updated_at = $2 WHERE session_id = $3 RETURNING *',
      ['ended', new Date().toISOString(), session_id]
    );
        ...
        if (session && session.status === 'waiting') {
          ...
          pool.connect().then(client => {
            client.query(
              `INSERT INTO chat_messages (message_id, session_id, sender_id, sender_type, message, created_at) 
               VALUES ($1, $2, $3, $4, $5, $6)`,
              [welcomeMessage.message_id, sessionId, 'agent-1', 'agent', welcomeMessage.message, welcomeMessage.timestamp]
            ).catch(console.error).finally(() => client.release());
          }).catch(console.error);
        }
      }
    ...
    try {
      ...
      if (message.type === 'chat_message' && ws.sessionId) {
        ...
        await client.query(
          `INSERT INTO chat_messages (message_id, session_id, sender_id, sender_type, message, created_at) 
           VALUES ($1, $2, $3, $4, $5, $6)`,
          [messageId, ws.sessionId, ws.userId || 'guest', 'user', message.message, timestamp]
        );
        client.release();
          ...
          if (ws.readyState === WebSocket.OPEN && sessionConns) {
            ...
            try {
              ...
              await client.query(
                `INSERT INTO chat_messages (message_id, session_id, sender_id, sender_type, message, created_at) 
                 VALUES ($1, $2, $3, $4, $5, $6)`,
                [agentMessageId, ws.sessionId, 'agent-1', 'agent', randomResponse, agentTimestamp]
              );
              client.release();
